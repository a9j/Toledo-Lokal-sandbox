-- Phase 5 fix: Ask Toledo could not answer the question Phase 5 exists for.
--
-- A development registers as a place entity, so it lands in citygraph_search's
-- scoped set and its filing shows up in the changes bucket. But there was no
-- developments bucket, which meant "what is being built in East Toledo" came
-- back as a line of change log text with no status, no developer and no date.
-- The model would then have nothing to answer with, which under the Phase 3
-- rule ("every fact must come from the search results") means it correctly
-- says it does not know about projects the database is holding.
--
-- This adds the bucket. Everything else about the function is unchanged.
