-- City OS Phase 0, Step 2, second fix: a street with a city on it still
-- answered with somebody's house.
--
-- The first fix (20260908000400) gated the ILIKE arm on the query carrying a
-- house number, which stopped "Jefferson Ave" and "43605". It left the
-- similarity arm open to every query, so:
--
--   resolve_address('Broadway St, Toledo') -> "100 Broadway St, Toledo, OH 43609"
--                                             match parcel_fuzzy, confidence 0.59
--
-- Same defect, one shape further out. Trigram similarity between a street and
-- a full address on that street is high enough to clear any threshold worth
-- having, so the threshold was never the right control. The rule now is about
-- what was asked, not how close the text is:
--
--   A query with a house number can match a parcel loosely, because the person
--   is naming a building. Typos are tolerated here (similarity 0.35), which
--   also fixes "100 Jeffersn Ave" returning nothing at all.
--
--   A query without a house number can only match a parcel exactly, or as a
--   prefix of one. Otherwise it is a street or a zip, and it gets a
--   neighborhood and no address.
--
-- The point is that resolve_address never invents a house number the person
-- did not type. A caller that saves the answer as a home address must not be
-- handed a house nobody named.
...
begin
  ...
  -- "Broadway St, Toledo" no, "43605" no.
  ...
  return query
    select p.id, p.address, p.neighborhood_id, n.name,
      ...
      left join public.neighborhoods n on n.id = p.neighborhood_id
     where case
             -- Named a building: match loosely, tolerate a typo.
             when v_numbered then
               p.address ilike '%' || v_esc || '%' escape '\'
